...
- `agent-browser set device` keeps `(hover: hover)`. For touch-only states, such as the contents popover on a wide tablet, emulate `hover: none` and disable scripts through `agent-browser get cdp-url` and one raw CDP connection; the overrides end when that connection closes.
